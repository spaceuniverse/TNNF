...
OPTIONS = OptionsStore(learnStep=0.0001,
                       rmsProp=0.9,
                       mmsmin=1e-6,
                       minibatch_size=1,
                       CV_size=1)
...
for iteration in xrange(EPOCHS):
    ...
    for i in xrange(data.shape[0] - WINDOW - 1):
        accumulate.append(RNN.architecture[0].A_predict.get_value()[1])
    ...
    #fig.savefig("sinonly.png")
    #
    stop = time.time()
    print str(iteration) + "\titeration time: " + str(stop - start)
    print "null A"
    Graph.Builder(name="accumulate_test.png", accuracy=accumulate, legend_on=True)
    # DEBUG
    '''
