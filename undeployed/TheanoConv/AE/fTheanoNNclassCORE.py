...
class FunctionModel(object):
    ...
    def MaxOut(W, X, B, E, *args):
        ...
        components, updates = theano.scan(fn=lambda n, weights, bias, data, activation: T.max(
            T.concatenate([data.T * T.dot(T.shape_padleft(weights[n, :]).T, E).T, (bias[n, 0] * E).T], axis=1), axis=1),
                                          sequences=neurons, non_sequences=[W, B, X, A], outputs_info=None)
        return components
...
class OptionsStore(object):
    def __init__(self, learnStep=0.01, regularization=False, lamda=0.001, sparsity=False, sparsityParam=0.01, beta=0.01,
                 dropout=False, dropOutParam=(1, 1), rmsProp=False, decay=0.9, mmsmin=1e-10, rProp=False):
...
class TheanoNNclass(object):
    ...
    def __init__(self, architecture, options, modelFunction=(FunctionModel.Sigmoid, FunctionModel.Sigmoid)):
        ...
        for i in xrange(self.lastArrayNum):
            w = theano.shared((np.random.randn(architecture[i + 1], architecture[i]) * 2 * random - random).astype(
                theano.config.floatX), name="w%s" % (i + 1))
    ...
    def trainCompile(self, numpatches):
        ...
        if self.options.dropout:
            ...
            self.dropOutVectors = []
            for i in xrange(self.lastArrayNum):
                self.dropOutVectors.append(
                    srng.binomial(p=self.options.dropOutParam[i], size=(self.architecture[i], 1)).astype(
        ...
        if self.options.sparsity and self.lastArrayNum <= 2:
            ...
            KL = T.sum(self.options.sparsityParam * T.log(self.options.sparsityParam / sprs) + (
                1 - self.options.sparsityParam) * T.log((1 - self.options.sparsityParam) / (1 - sprs)))
        ...
        if self.options.rProp:
            ...
            self.deltaW = []
            for i in xrange(len(self.derivativesArray)):
                prevGW = theano.shared(np.tile(1.0, self.gradArray[i].get_value().shape).astype(theano.config.floatX),
                                       name="prewGW%s" % (i + 1))
                deltaW = theano.shared(np.tile(np.float32(baseRpropStep), self.gradArray[i].get_value().shape).astype(
                    theano.config.floatX), name="deltaW%s" % (i + 1))
        ...
        for i in xrange(len(self.derivativesArray)):
            if self.options.rmsProp:
                ...
                updateVar = self.deltaW[i] * decreaseCoef * T.lt(self.prevGW[i] * self.derivativesArray[i], 0) + \
                            self.deltaW[i] * increaseCoef * T.gt(self.prevGW[i] * self.derivativesArray[i], 0)
        ...
        self.train = theano.function(inputs=[self.x, self.y], outputs=[self.cost, XENT], updates=self.updatesArray,
